...
bool LaneDetector::init()
{
  ...
  if (obs_el_size % 2 == 0)
    obs_el_size++;
  obstacle_element_ =
      getStructuringElement(2, cv::Size(obs_el_size, obs_el_size), cv::Point(obs_el_size / 2, obs_el_size / 2));
...
void LaneDetector::imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  cv::adaptiveThreshold(homography_frame_, binary_frame_, 255, cv::ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY,
                        treshold_block_size_, threshold_c_);
  ...
  if (!init_imageCallback_)
  {
    ...
    cv::HoughLinesP(outside_road_, lines_out_h_, isec_HL_dist_res_, isec_HL_angle_res_, isec_HL_thresh_,
                    isec_HL_min_length_, isec_HL_max_gap_);
  }
  else
  {
  ...
  else
  {
    ...
    if (!isIntersection())
    {
      if (waiting_for_stabilize_)
      {
        ...
        if (right_line_.isExist() && right_line_.getPoints()[0].x < half_of_image &&
                right_line_.getPoints()[right_line_.pointsSize()].x > half_of_image ||
            left_line_.isExist() && left_line_.getPoints()[0].x < half_of_image &&
                left_line_.getPoints()[left_line_.pointsSize()].x > half_of_image ||
            center_line_.isExist() && center_line_.getPoints()[0].x < half_of_image &&
                center_line_.getPoints()[center_line_.pointsSize()].x > half_of_image)
        {
          if (right_line_.getMaxDiffonX() < 0.3 && left_line_.getMaxDiffonX() < 0.3 &&
              center_line_.getMaxDiffonX() < 0.3)
          {
...
{
  cv::warpPerspective(input_frame, homography_frame, topview2cam_, topview_size_,
...
void LaneDetector::mergeMiddleLines()
{
  for (int i = 0; i < lines_vector_converted_.size(); ++i)
  {
    ...
    for (int j = i + 1; j < lines_vector_converted_.size(); ++j)
    {
      if (!init_imageCallback_)
      {
        if (lines_vector_converted_[j][0].y <
                getPolyY(right_line_.getCoeff(), lines_vector_converted_[j][0].x) + 0.05 ||
      ...
      float distance =
          std::abs(a * lines_vector_converted_[j][0].x - lines_vector_converted_[j][0].y + b) / sqrtf(a * a + 1);
      float distance2 =
          getDistance(lines_vector_converted_[j][0], lines_vector_converted_[i][lines_vector_converted_[i].size() - 1]);

      if (distance < max_mid_line_distance_ && distance2 < max_mid_line_gap_)
      {
        lines_vector_converted_[i].insert(lines_vector_converted_[i].end(), lines_vector_converted_[j].begin(),
                                          lines_vector_converted_[j].end());
...
void LaneDetector::dynamicMask(cv::Mat& input_frame, cv::Mat& output_frame)
{
  ...
  std::vector<cv::Point2f> left_line_offset =
      createOffsetLine(left_line_.getCoeff(), left_line_.getDegree(), offset_left);
  std::vector<cv::Point2f> right_line_offset =
      createOffsetLine(right_line_.getCoeff(), right_line_.getDegree(), offset_right);
...
void LaneDetector::ROILaneRight(cv::Mat& input_frame, cv::Mat& output_frame)
{
  ...
  std::vector<cv::Point2f> center_line_offset =
      createOffsetLine(center_line_.getCoeff(), center_line_.getDegree(), offset_center);
  std::vector<cv::Point2f> right_line_offset =
      createOffsetLine(right_line_.getCoeff(), right_line_.getDegree(), offset_right);
...
void LaneDetector::initRecognizeLines()
{
  ...
  for (int i = 0; i < lines_vector_converted_.size(); ++i)
  {
    if (std::abs(nominal_center_line_Y_ - lines_vector_converted_[i][0].y) < min &&
        lines_vector_converted_[i][0].x < 1 && cv::arcLength(lines_vector_converted_[i], false) > 0.15)
    {
  ...
  for (int i = 0; i < lines_vector_converted_.size(); ++i)
  {
    if (lines_vector_converted_[i][0].y < center_line_.getPoints()[0].y && lines_vector_converted_[i][0].x < 1 &&
        cv::arcLength(lines_vector_converted_[i], false) > 0.2)
    {
...
void LaneDetector::linesApproximation()
{
  ...
  if (debug_mode_)
  {
    ...
    std::cout << "right_length  " << right_line_.getLength() << std::endl << std::endl;

    std::cout << "left_degree   " << left_line_.getDegree() << std::endl;
    std::cout << "center_degree " << center_line_.getDegree() << std::endl;
    std::cout << "right_degree  " << right_line_.getDegree() << std::endl << std::endl;
...
void LaneDetector::generatePoints()
{
  for (int k = 0; k < lines_vector_converted_.size(); ++k)
  {
    ...
    {
      float distance = getDistance(lines_vector_converted_[k][i], lines_vector_converted_[k][i + 1]);
      if (distance > 1 / points_density_)
      {
        ...
        float x_dif = (lines_vector_converted_[k][i + 1].x - lines_vector_converted_[k][i].x) / (add + 1);
        float y_dif = (lines_vector_converted_[k][i + 1].y - lines_vector_converted_[k][i].y) / (add + 1);
        for (int j = 0; j < add; ++j)
        {
          p.x = x1 + x_dif * (j + 1);
          p.y = y1 + y_dif * (j + 1);
...
void LaneDetector::adjust(RoadLine& good_road_line, RoadLine& short_road_line, bool left_offset)
{
  ...
  {
    if (polyfit(createOffsetLine(good_road_line.getCoeff(), good_road_line.getDegree(), offset),
                short_road_line.getDegree(), tmp_coeff))
    {
...
void LaneDetector::calcRoadWidth()
{
  ...
  if (center_line_.getCoeff().size() - 1 == 3)
    deriative = 3 * center_line_.getCoeff()[3] * pow(p_ahead.x, 2) + 2 * center_line_.getCoeff()[2] * p_ahead.x +
                center_line_.getCoeff()[1];
  else if (center_line_.getCoeff().size() - 1 == 2)
    deriative = 2 * center_line_.getCoeff()[2] * p_ahead.x + center_line_.getCoeff()[1];
  else
...
std::vector<cv::Point2f> LaneDetector::createOffsetLine(const std::vector<float>& coeff, const int& degree,
                                                        float offset)
{
...
void LaneDetector::detectStartAndIntersectionLine()
{
  ...
  if (debug_mode_)
  {
    ...
    for (int i = 0; i < right_lines.size(); i++)
    {
      ...
      cv::Point2f vertices2f[4];
      rect.points(vertices2f);
      // ROS_INFO("1: %.4f       2: %.4f    3: %.4f   4: %.4f", vertices2f[0].x, vertices2f[1].x, vertices2f[2].x,
      // vertices2f[3].x);
...
void LaneDetector::drawParticles(int num)
{
  ...
  for (int i = 0; i < num; ++i)
  {
    if (center_line_.isPFInitialized())
    {
      ...
      {
        cv::line(pf_vis_mat_, pf_line[j - 1], pf_line[j], cv::Scalar(color_set[i][0], color_set[i][1], color_set[i][2]),
                 1.5, cv::LINE_AA);
      }
...
{
  // resolve 'this':
  LaneDetector* that = (LaneDetector*)ptr;  // NOLINT
...
void LaneDetector::tuneParams(const ros::TimerEvent& time)
{
  ...
  cv::adaptiveThreshold(homography_gray, test_threshold_c_mat, 255, cv::ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY,
                        treshold_block_size_, threshold_c_);
...
bool LaneDetector::isIntersection()
{
  ...
  if (right_line_.isExist())
  {
    ...
    {
      cv::Point2f p_center =
          cv::Point2f(lines_out_h_world_[i].x, getPolyY(center_line_.getCoeff(), lines_out_h_world_[i].x));
      ...
      if (min_dist < isec_line_close_)
      {
        ...
        float a2 = atan2(p_line.y - right_line_.getPoints()[0].y, p_line.x - right_line_.getPoints()[0].x);
  ...
  if (left_line_.isExist())
  {
    for (int i = 0; i < lines_out_h_world_.size(); i += 2)
    {
      ...
      if (min_dist < isec_line_close_)
      {
        ...
        float a2 = atan2(p_line.y - left_line_.getPoints()[0].y, p_line.x - left_line_.getPoints()[0].x);
        float a = std::abs(std::abs(a1 - a2) - 1.57);
