    ...
    def callback(self, data):
        ...
        if only_once:
            ...
            pixels = np.float32([[topview_size[0], topview_size[1]],
                                 [0, topview_size[1]], [topview_size[0], 0],
                                 [0, 0]])
            ...
            homography = cv2.warpPerspective(
                cv_image,
                topview2cam_, (topview_size[0], topview_size[1]),
                flags=cv2.INTER_CUBIC | cv2.WARP_INVERSE_MAP)

            ret, thresh = cv2.threshold(homography, 0, 255,
                                        cv2.THRESH_BINARY_INV)

            fs_write = cv2.FileStorage(yaml_output_file,
