...
void RoadLinesVisual::setMessage(const custom_msgs::RoadLines::ConstPtr& msg)
{
  ...
  for (x = start_x_; x < end_x_; x += step_x_)
  {
    ...
    if (x != start_x_)
    {
      ...
      left_segment->setColor(boundaries_color_.r, boundaries_color_.g, boundaries_color_.b, alpha_);

      center_segment->setColor(centerline_color_.r, centerline_color_.g, centerline_color_.b, alpha_);

      right_segment->setColor(boundaries_color_.r, boundaries_color_.g, boundaries_color_.b, alpha_);
...
void RoadLinesVisual::setRenderingRangeAndStep(float start_x, float end_x, float step_x)
{
  start_x_ = start_x;
  end_x_ = end_x;
  step_x_ = step_x;
