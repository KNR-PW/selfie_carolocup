...
DriveClient::DriveClient(std::string name, const ros::NodeHandle& pnh) : ac_(name, true), pnh_(pnh)
{
...
void DriveClient::setGoal(boost::any goal)
{
  ...
  ac_.sendGoal(goal_, boost::bind(&DriveClient::doneCb, this, _1, _2), boost::bind(&DriveClient::activeCb, this));
...
void DriveClient::prepareAction()
{
  ...
  ROS_INFO(
      "Prepare drive - vision reset, reset Lane control, MuxSelect drive/lane_control,"
      "avoinding obst set %d",
      drive_mode_);
}
