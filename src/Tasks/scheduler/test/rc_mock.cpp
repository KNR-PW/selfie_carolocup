...
class Rc_mock
{
  ...
  ros::Timer timer_autonomous_pub_ =
      nh_.createTimer(ros::Duration(1.0 / 10.0), std::bind(&Rc_mock::set_switch_state, this, RC_AUTONOMOUS));
...
public:
  Rc_mock() : pnh_("~")
  {
    ...
    if (rc_reset_time_ > 0)
    {
      timer_rc_reset_ = nh_.createTimer(ros::Duration(rc_reset_time_), std::bind(&Rc_mock::rc_reset, this));
    }
  }
