 ...
 **/
#include "starting_procedure/qr_decoder.h"
#include <algorithm>
#include <vector>

QrDecoder::QrDecoder(const ros::NodeHandle& nh, const ros::NodeHandle& pnh)
  : nh_(nh)
  , pnh_(pnh)
  , visualization_(false)
  , min_detect_rate_(0.4)
  , iterations_to_valid_(2)
  , detect_rate_(1.0)
  , count_frame_(0)
  , count_bar_(0)
...
void QrDecoder::calcRate(const ros::TimerEvent& time)
{
  if (count_frame_ != 0)
  {
    detect_rate_ = count_bar_ / static_cast<float>(count_frame_);
  }
  else
  {
    detect_rate_ = 0;
  ...
  count_bar_ = 0;
  count_frame_ = 0;
  if (count_valid_iterations_ < iterations_to_valid_)
  {
