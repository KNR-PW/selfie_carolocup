 ...
 **/
#include "starting_procedure/gate_scanner.h"
#include <vector>

GateScanner::GateScanner(const ros::NodeHandle& nh, const ros::NodeHandle& pnh) : nh_(nh), pnh_(pnh)
{
  ...
  if (min_gate_seen_count_ < 1)
    min_gate_seen_count_ = 1;
  gate_seen_count_ = 0;
}
...
void GateScanner::laserScanCB(const sensor_msgs::LaserScan& msg)
{
  ...
  {
    if (*it > msg.range_max || *it < msg.range_min)
      continue;
