...
def get_images(goal_folder):
    ...
    client = actionlib.SimpleActionClient("/image_file_sequencer",
                                          GrabSequenceAction)
    client.wait_for_server()
    rospy.loginfo("Got server")

    goal = GrabSequenceGoal()
    goal.desiredExposureTimes = [
        40, 700, 7000
    ...
    for i in range(len(result.exposureTimes)):
        file_name = goal_folder + '/' + "img_" + str(
            int(result.exposureTimes[i])) + ".png"
        print file_name
        mat = bridge.imgmsg_to_cv2(result.images[i], "mono8")
        cv2.imwrite(file_name, mat)

    rospy.loginfo("Wrote " + str(len(result.exposureTimes)) + " images to " +
                  goal_folder)
