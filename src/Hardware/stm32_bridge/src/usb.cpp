...
void USB_STM::fill_publishers(Pub_messages &pub_data)
{
  ...
  pub_data.imu_msg.orientation.x = static_cast<float>(read_frame->x / 32767.f);
  pub_data.imu_msg.orientation.y = static_cast<float>(read_frame->y / 32767.f);
  pub_data.imu_msg.orientation.z = static_cast<float>(read_frame->z / 32767.f);
  pub_data.imu_msg.orientation.w = static_cast<float>(read_frame->w / 32767.f);

  pub_data.imu_msg.linear_acceleration.x = static_cast<float>(read_frame->acc[0] / 8192.f * 9.80655f);
  pub_data.imu_msg.linear_acceleration.y = static_cast<float>(read_frame->acc[1] / 8192.f * 9.80655f);
  pub_data.imu_msg.linear_acceleration.z = static_cast<float>(read_frame->acc[2] / 8192.f * 9.80655f);

  pub_data.imu_msg.angular_velocity.x = static_cast<float>(read_frame->rates[0] / 65.535f);
  pub_data.imu_msg.angular_velocity.y = static_cast<float>(read_frame->rates[1] / 65.535f);
  pub_data.imu_msg.angular_velocity.z = static_cast<float>(read_frame->rates[2] / 65.535f);

  // acctual car velocity
  pub_data.velo_msg.data = static_cast<float>(read_frame->velocity) / 1000.0;

  // distance from encoders
  pub_data.dist_msg.data = static_cast<float>(read_frame->distance) / 1000.0;
