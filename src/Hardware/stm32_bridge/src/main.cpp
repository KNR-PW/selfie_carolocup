...
#include <stm32_bridge/Stm32BridgeConfig.h>
...
void reconfigureCB(stm32_bridge::Stm32BridgeConfig &config, uint32_t level);
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n;
    ros::NodeHandle pnh("~");
    dynamic_reconfigure::Server<stm32_bridge::Stm32BridgeConfig> dr_server_;
    dynamic_reconfigure::Server<stm32_bridge::Stm32BridgeConfig>::CallbackType dr_server_CB_;
...
void reconfigureCB(stm32_bridge::Stm32BridgeConfig &config, uint32_t level)
{
    if (ackermann_offset_back != static_cast<float>(config.ackermann_offset_back))
    {
        ackermann_offset_back = static_cast<float>(config.ackermann_offset_back);
        ROS_INFO("ackermann_offset_back new value: %f", ackermann_offset_back);
    }
    if (ackermann_offset_front != static_cast<float>(config.ackermann_offset_front))
    {
        ackermann_offset_front = static_cast<float>(config.ackermann_offset_front);
        ROS_INFO("ackermann_offset_front new value: %f", ackermann_offset_front);
    }

    if (parallel_offset_back_right != static_cast<float>(config.parallel_offset_back_right))
    {
        parallel_offset_back_right = static_cast<float>(config.parallel_offset_back_right);
        ROS_INFO("parallel_offset_back_right end new value %f", parallel_offset_back_right);
    }
    if (parallel_offset_front_right != static_cast<float>(config.parallel_offset_front_right))
    {
        parallel_offset_front_right = static_cast<float>(config.parallel_offset_front_right);
        ROS_INFO("parallel_offset_front_right end new value %f", parallel_offset_front_right);
    }
    if (parallel_offset_back_left != static_cast<float>(config.parallel_offset_back_left))
    {
        parallel_offset_back_left = static_cast<float>(config.parallel_offset_back_left);
        ROS_INFO("paralell_offset_back_left end new value %f", parallel_offset_back_left);
    }
    if (parallel_offset_front_left != static_cast<float>(config.parallel_offset_front_left))
    {
        parallel_offset_front_left = static_cast<float>(config.parallel_offset_front_left);
        ROS_INFO("parallel_offset_front_left end new value %f", parallel_offset_front_left);
    }

    if (front_axis_offset != static_cast<float>(config.front_axis_offset))
    {
        front_axis_offset = static_cast<float>(config.front_axis_offset);
        ROS_INFO("front_axis_offset end new value %f", front_axis_offset);
    }
    if (back_axis_offset != static_cast<float>(config.back_axis_offset))
    {
        back_axis_offset = static_cast<float>(config.back_axis_offset);
